...
TG_MY_CHANNEL  = os.environ.get("TG_MY_CHANNEL", "")
...
def _notify(text: str):
    if TG_NOTIFY_CHAT and TG_BOT_TOKEN:
        try:
...
def _pick_best_news(conn: sqlite3.Connection, min_score: int) -> dict | None:
    """Выбрать лучшую непубликовавшуюся новость за последние 12ч."""
...
def _publish_to_channel(text: str) -> tuple[bool, int | None]:
    if not TG_BOT_TOKEN or not TG_MY_CHANNEL:
        ...
        return False, None
    try:
        result = _send_text(TG_MY_CHANNEL, text)
        if result.get("ok"):
...
def run_auto_post(db_path: Path | None = None) -> dict:
    """

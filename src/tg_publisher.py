...
TOPIC_EMOJI = [
    ...
    (["энергетик", "коммунал", "жкх", "свет", "газ", "отключен", "электр"],      "⚡️"),
]
...
def _two_emoji(text: str) -> str:
    t = (text or "").lower()
    matched = [e for kws, e in TOPIC_EMOJI if any(k in t for k in kws)]
    if len(matched) >= 2:
...
def _format_post(item: dict) -> str:
    ...
    combined = (item.get("translated_text") or "") + " " + headline + " " + summary
    prefix   = _two_emoji(combined)

    channel = TG_MY_CHANNEL.lstrip("@")
    tagline = os.environ.get("TG_CHANNEL_TAGLINE", "подпишись на свежие новости!")

    lines = []
    if headline:
        lines += [f"{prefix} <b>{headline}</b>", ""]
    if summary:
        lines += [summary, ""]
    lines += ["Распространите сообщение", "", f"👉 @{channel} — {tagline}"]
    if source:
